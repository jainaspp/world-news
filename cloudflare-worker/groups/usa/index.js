...
const FEEDS = [
    ...
    { name: 'Fox Economy', url: 'https://moxie.foxbusiness.com/google-publisher/economy.xml', region: 'USA', lang: 'en' },
    { name: 'Fox Markets', url: 'https://moxie.foxbusiness.com/google-publisher/markets.xml', region: 'USA', lang: 'en' },
    { name: 'Fox Tech', url: 'https://moxie.foxbusiness.com/google-publisher/technology.xml', region: 'USA', lang: 'en' },
...
export default {
    async scheduled(controller, env, ctx) {
        const t0 = Date.now();
        console.log('START [usa] ' + FEEDS.length + ' feeds');
        ...
        console.log('DONE [usa] +' + result.total + ' | ' + result.errors + ' errors | ' + (Date.now()-t0) + 'ms');
    },

    async fetch(request) {
        return new Response(JSON.stringify({ status: 'ok', group: 'usa', feeds: FEEDS.length }), {
