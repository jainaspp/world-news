/**
 * Worker: TaiwanHK (3 feeds) -> Supabase
 * Cron: */5 * * * *  (every 5 min)
 */
...
const FEEDS = [
    ...
    { name: 'RTHK HK', url: 'https://news.rthk.hk/rss/news', region: 'HKG', lang: 'zh' },
...
export default {
    async scheduled(controller, env, ctx) {
        const t0 = Date.now();
        console.log('START [tw_hk] ' + FEEDS.length + ' feeds');
        const links = await getExistingLinks();
        const result = await doCrawl(links);
        console.log('DONE [tw_hk] +' + result.total + ' | ' + result.errors + ' errors | ' + (Date.now()-t0) + 'ms');
    },

    async fetch(request) {
        return new Response(JSON.stringify({ status: 'ok', group: 'tw_hk', feeds: FEEDS.length }), {
