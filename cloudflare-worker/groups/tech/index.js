...
const FEEDS = [
    ...
    { name: 'Smashing Mag', url: 'https://www.smashingmagazine.com/feed/', region: 'TEC', lang: 'en' },
...
export default {
    async scheduled(controller, env, ctx) {
        const t0 = Date.now();
        console.log('START [tech] ' + FEEDS.length + ' feeds');
        ...
        console.log('DONE [tech] +' + result.total + ' | ' + result.errors + ' errors | ' + (Date.now()-t0) + 'ms');
    },

    async fetch(request) {
        return new Response(JSON.stringify({ status: 'ok', group: 'tech', feeds: FEEDS.length }), {
