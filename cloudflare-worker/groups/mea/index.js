...
const FEEDS = [
    ...
    { name: 'BBC LatAm', url: 'http://feeds.bbci.co.uk/news/world/latin_america/rss.xml', region: 'LAT', lang: 'en' }
];
...
export default {
    async scheduled(controller, env, ctx) {
        const t0 = Date.now();
        console.log('START [mea] ' + FEEDS.length + ' feeds');
        const links = await getExistingLinks();
        const result = await doCrawl(links);
        console.log('DONE [mea] +' + result.total + ' | ' + result.errors + ' errors | ' + (Date.now()-t0) + 'ms');
    },

    async fetch(request) {
        return new Response(JSON.stringify({ status: 'ok', group: 'mea', feeds: FEEDS.length }), {
