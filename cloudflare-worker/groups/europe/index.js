/**
 * Worker: Europe (4 feeds) -> Supabase
 * Cron: */5 * * * *  (every 5 min)
 */
...
export default {
    async scheduled(controller, env, ctx) {
        const t0 = Date.now();
        console.log('START [europe] ' + FEEDS.length + ' feeds');
        const links = await getExistingLinks();
        const result = await doCrawl(links);
        console.log('DONE [europe] +' + result.total + ' | ' + result.errors + ' errors | ' + (Date.now()-t0) + 'ms');
    },

    async fetch(request) {
        return new Response(JSON.stringify({ status: 'ok', group: 'europe', feeds: FEEDS.length }), {
