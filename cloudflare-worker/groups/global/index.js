/**
 * Worker: Global (9 feeds) -> Supabase
 * Cron: */5 * * * *  (every 5 min)
 */
...
const FEEDS = [
    ...
    { name: 'The Guardian', url: 'https://www.theguardian.com/world/rss', region: 'ALL', lang: 'en' },
    { name: 'Fox Business', url: 'https://moxie.foxbusiness.com/google-publisher/latest.xml', region: 'ALL', lang: 'en' }
];
...
export default {
    async scheduled(controller, env, ctx) {
        const t0 = Date.now();
        console.log('START [global] ' + FEEDS.length + ' feeds');
        const links = await getExistingLinks();
        const result = await doCrawl(links);
        console.log('DONE [global] +' + result.total + ' | ' + result.errors + ' errors | ' + (Date.now()-t0) + 'ms');
    },

    async fetch(request) {
        return new Response(JSON.stringify({ status: 'ok', group: 'global', feeds: FEEDS.length }), {
