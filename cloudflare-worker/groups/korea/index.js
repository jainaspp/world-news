...
export default {
    async scheduled(controller, env, ctx) {
        const t0 = Date.now();
        console.log('START [korea] ' + FEEDS.length + ' feeds');
        ...
        console.log('DONE [korea] +' + result.total + ' | ' + result.errors + ' errors | ' + (Date.now()-t0) + 'ms');
    },

    async fetch(request) {
        return new Response(JSON.stringify({ status: 'ok', group: 'korea', feeds: FEEDS.length }), {
